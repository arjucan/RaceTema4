...
```{r}
start_col <- which(names(cran) == "r_arch")
end_col <- which(names(cran) == "country")

cran[, start_col:end_col]
```
...
```{r}
start_col <- which(names(cran) == "date")
end_col <- which(names(cran) == "size")
...
```{r}
cran3 <- mutate(cran2, size_mb = size / (2^20))
cran3
```
